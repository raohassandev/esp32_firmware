(() => {
    ...
    function evaluate(payload) {
        ...
        return [
            ...
            check('engineering', 'Engineering access', session.development_auto_unlock || session.authenticated ? 'warn' : 'pass', session.development_auto_unlock ? 'Development auto-unlock is active.' : session.authenticated ? 'Engineering is currently unlocked.' : 'Engineering is locked and requires authentication.', 'Disable development auto-unlock before resale.'),
            check('development', 'Development provisioning', 'warn', 'Development Wi-Fi provisioning is intentionally configured for Rao / password123.', 'Remove committed development credentials and provisioning before production.')
        ];
    }
