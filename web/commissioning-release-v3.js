...
function channel(){const d=active();if(!d)return devices();const body=d.channel==='tcp'?`<div class="cr-grid"><label class="wide"><span>IP address or hostname</span><input id="crHost" value="${esc(d.tcp.host)}" placeholder="192.168.1.120"></label><label><span>TCP port</span><input id="crPort" type="number" value="${d.tcp.port}"></label><label><span>Unit ID</span><input id="crUnit" type="number" value="${d.tcp.unit_id}"></label>${d.type==='inverter'?`<label><span>Rated power (kW)</span><input id="crRated" type="number" step="any" min="0" value="${d.rated_kw??''}" placeholder="Nameplate AC rating"></label>`:`<label><span>Enforce the grid limit on</span><select id="crBasis"><option value="0" ${Number(d.phase_basis??0)===0?'selected':''}>Lowest phase (stricter)</option><option value="1" ${Number(d.phase_basis??0)===1?'selected':''}>Total power</option></select></label>${supplyIsDual()?`<label><span>What this meter measures</span><select id="crRole">${METER_ROLES.map(([v,l])=>`<option value="${v}" ${Number(d.role||1)===v?'selected':''}>${l}</option>`).join('')}</select></label>${Number(d.role||1)===2?`<label><span>Which generator</span><input id="crGenIndex" type="number" min="0" max="2" value="${Number(d.generator_index)||0}"></label>`:''}`:`<label class="wide"><span>What this meter measures</span><p class="cr-role-fixed">Whichever supply is live. On a one-meter plant this instrument sits on the changeover and its tariff input says which — so there is nothing to choose here, and choosing would be a label that is wrong for half of every day.</p></label>`}`}</div>`:`<div class="cr-notice warn"><strong>RTU runtime not released</strong><span>Parameters may be prepared, but the device cannot pass readiness until the ESP32 RS-485 master is implemented and qualified.</span></div><div class="cr-grid"><label><span>RS-485 port</span><select id="crUart"><option value="1" ${d.rtu.uart===1?'selected':''}>RS-485 1</option><option value="2" ${d.rtu.uart===2?'selected':''}>RS-485 2</option></select></label><label><span>Baud rate</span><select id="crBaud">${[9600,19200,38400,57600,115200].map(v=>`<option ${d.rtu.baud===v?'selected':''}>${v}</option>`).join('')}</select></label><label><span>Parity</span><select id="crParity">${['none','even','odd'].map(v=>`<option ${d.rtu.parity===v?'selected':''}>${v}</option>`).join('')}</select></label><label><span>Data bits</span><select id="crDataBits"><option ${d.rtu.data_bits===8?'selected':''}>8</option><option ${d.rtu.data_bits===7?'selected':''}>7</option></select></label><label><span>Stop bits</span><select id="crStopBits"><option ${d.rtu.stop_bits===1?'selected':''}>1</option><option ${d.rtu.stop_bits===2?'selected':''}>2</option></select></label><label><span>Slave ID</span><input id="crRtuUnit" type="number" value="${d.rtu.unit_id}"></label></div>`;return`<section class="cr-stage"><div class="cr-head"><p class="eyebrow">Physical and network connection</p><h2>Communication channel</h2><p>Select how each device connects. Only fields relevant to the chosen channel are displayed.</p></div>${supplyTopology()}<div class="cr-layout">${tabs()}<article class="cr-editor"><div class="cr-editor-head"><div><p>${esc(d.brand)} · ${esc(d.model)}</p><h3>Connection settings</h3></div><span>${d.type}</span></div><div class="cr-grid"><label class="wide"><span>Device name</span><input id="crDeviceName" value="${esc(d.name)}"></label><label><span>Channel</span><select id="crChannel"><option value="tcp" ${d.channel==='tcp'?'selected':''}>Modbus TCP</option><option value="rtu" ${d.channel==='rtu'?'selected':''}>Modbus RTU</option></select></label></div>${body}</article></div></section>${nav()}`;}
/* HOW MANY METERS MEASURE THE SUPPLY.
 *
 * This was a step of its own -- "Source detection", ninth of nine -- and it did
 * not deserve one. It asks a single question about how the plant is wired, and
 * the answer is already implied by what the engineer has just configured on
 * this very page: one meter, or two with a role each.
 *
 * ONE METER is the tariff installation. A single EM500 on the changeover, and
 * its tariff register says which supply it is measuring: 1 grid, 2 generator.
 * Nothing further is asked, because nothing further is knowable from one
 * instrument.
 *
 * TWO METERS means each is permanently on one supply, so the question becomes
 * WHICH -- and that is the "What this meter measures" field already on this
 * page, per meter. The controller compares their measured power to decide which
 * is carrying the plant.
 *
 * Saved immediately on change rather than on a Continue, because it is one
 * value and a step that exists only to hold a Save button is the step this
 * replaces.
 */
/* The chosen topology, remembered so the fields below can follow it. Defaults
 ...
 */
/* The Source detection step is gone. It asked one question about how the plant is wired, and that question now sits on the Channel step beside the meters it is about -- see supplyTopology(). The full EM-500 panel it hosted remains reachable from the meter analyser for the rare plant that needs a register or threshold changed. */
...
async function loadInverters(){const target=$('crInverterCatalog');if(!target)return;if(!access()?.mayRequest('/api/inverter-profiles')){target.innerHTML='<div class="cr-empty">Unlock Engineering to load the inverter catalogue.</div>';return;}try{const payload=await api('/api/inverter-profiles');const profiles=payload.profiles||[];state.inverterProfiles=profiles;target.innerHTML=profiles.length?`<div class="cr-picker"><select id="crInverterPick" aria-label="Inverter model">${profiles.map(p=>`<option value="${esc(String(p.id||p.profile_id))}">${esc(p.manufacturer||p.brand||'')} \u2014 ${esc(p.model_family||p.model||p.id)}</option>`).join('')}</select><button class="button primary" data-action="add-inverter">Add</button></div><p class="cr-picker-note" id="crInverterNote"></p>`:'<div class="cr-empty">The controller reported no inverter profiles.</div>';bindInverterPicker();}catch(error){target.innerHTML=`<p>${esc(error.message)}</p>`;}}
...
/* ------------------------------------------------------------- the pickers
 *
 * A picker is a picker: choose one, add it, move on. What each one must also do
 * is say what the choice MEANS before it is made -- a lab-simulator profile, a
 * brand deferred this phase and a family with no transcribed register map all
 * looked identical as cards, and the difference decides whether this firmware
 * may command the machine or only read it.
 */
function inverterProfileNote(profile){
 if(!profile)return'';
 if(profile.simulator_only)return'Lab simulator profile. Commandable only against an endpoint declared a simulator; this is not evidence about physical equipment.';
 if(profile.deferred_this_phase)return`Not commandable in this phase${profile.deferred_reason?`: ${profile.deferred_reason}`:'.'}`;
 if(profile.has_register_map===false)return'No transcribed register map. It commissions as a manual endpoint, and the registers will be yours rather than the manufacturer\'s.';
 return profile.manual_reference?`Transcribed from: ${profile.manual_reference}`:'Register map available.';}

function updateInverterNote(){
 const note=$('crInverterNote'),pick=$('crInverterPick');
 if(!note||!pick)return;
 note.textContent=inverterProfileNote((state.inverterProfiles||[]).find(p=>String(p.id||p.profile_id)===pick.value));}

function bindInverterPicker(){
 const pick=$('crInverterPick');
 if(!pick)return;
 pick.addEventListener('change',updateInverterNote);
 document.querySelector('[data-action="add-inverter"]')?.addEventListener('click',()=>{
  const profile=(state.inverterProfiles||[]).find(p=>String(p.id||p.profile_id)===pick.value);
  if(!profile)return;
  addDevice({id:String(profile.id||profile.profile_id),type:'inverter',
   brand:profile.manufacturer||profile.brand||'',
   model:profile.model_family||profile.model||'',
   protocols:['tcp'],
   /* Same rule the cards used: only a profile with a transcribed map counts as
    * verified. Everything else commissions as a manual endpoint and says so. */
   verified:profile.has_register_map===true});});
 updateInverterNote();}

function updateMeterNote(){
 const note=$('crMeterNote'),pick=$('crMeterPick');
 if(!note||!pick)return;
 const profile=catalog.find(p=>p.id===pick.value);
 note.textContent=profile?(profile.verified
   ?'Register map verified for this model.'
