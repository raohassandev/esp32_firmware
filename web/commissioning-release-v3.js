...
function crConfigRoundTrip(){const written=['enabled','name','host','port','unit_id','timeout_ms','poll_ms','function','active_power_address','data_type','word_order','scale'];const read=['enabled','name','host','port','unit_id','timeout_ms','poll_ms','function','active_power_address','data_type','word_order','scale'];return written.filter(k=>!read.includes(k));}
...
function updateChannel(){const d=active();if(!d)return;d.name=$('crDeviceName')?.value.trim()||d.name;d.channel=$('crChannel')?.value||d.channel;if(d.channel==='tcp'){d.tcp.host=$('crHost')?.value.trim()||'';d.tcp.port=Number($('crPort')?.value);d.tcp.unit_id=Number($('crUnit')?.value);
  ...
  * "entered as zero". */
 if($('crRated')){const raw=$('crRated').value.trim();d.rated_kw=raw===''?undefined:Number(raw);}}else{d.rtu.uart=Number($('crUart')?.value);d.rtu.baud=Number($('crBaud')?.value);d.rtu.parity=$('crParity')?.value;d.rtu.data_bits=Number($('crDataBits')?.value);d.rtu.stop_bits=Number($('crStopBits')?.value);d.rtu.unit_id=Number($('crRtuUnit')?.value);}save();}
