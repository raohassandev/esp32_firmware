...
Audit baseline for this reconciliation: `dev` at `9e14f9e1d43c961a6f3d5befd49389c7df94c829` after governed merges through PR #110.
...
| R-SRC-01 | Grid/Generator/Sync/Transfer/Island/NoSource/Conflict/Unknown modes | ChatGPT | CI + bench | SOFTWARE READY ON HELD DRAFT / bench gated | Draft PR #106 head `a1620789235d21b515f9f245f2329fab88b50558`; #80 physical gate before merge |
| R-SRC-02 | fresh dwell on carrying-source transition | ChatGPT | CI + bench | SOFTWARE READY ON HELD DRAFT / bench gated | Draft PR #106 exact head; #80 |
| R-SRC-03 | fail closed on Transfer/Conflict/Stale | ChatGPT | CI + bench | SOFTWARE READY ON HELD DRAFT / bench gated | Draft PR #106 exact head; #80 |
...
| R-INV-01 | production profile manual/identity mapping | ChatGPT + Owner | CI + bench | BLOCKED PER MANUFACTURER/MODEL | #82; exact official manual + model/firmware + bench identity required, simulator maps are not evidence |
| R-INV-02 | command width/scale/range/finite validation | ChatGPT | CI + bench | GENERIC SOFTWARE CONTRACT COMPLETE / per-profile evidence remains | PR #108 merge `b8996d5f834cc8edeb084f56c802ff5fc6ecd04d`; exact-head focused + full ESP-IDF CI GREEN; #82 for manufacturer values |
| R-INV-03 | write/readback/tolerance/rollback/safe-zero | ChatGPT + Claude | CI + bench | GENERIC CORE VERIFIED / BLOCKED PER PROFILE | `inverter_runtime_write_gate_source_contract.py` + PR #108 metadata/width gate; physical write/readback/rollback remains #82/#83 |
| R-INV-04 | reconnect identity reverification | ChatGPT | CI + bench | SOFTWARE CONTRACT COMPLETE / per-profile bench remains | PR #102 merge `b60bcb9474f4ed7dd0b0ed631410b54628a47d01`; failed/stale telemetry revokes identity and periodic recheck is enforced; #82 |
...
| R-WEB-03 | Engineering error DOM stability / no recursive observer freeze | ChatGPT | GitHub Actions | COMPLETE/MERGED REGRESSION GATE | PR #105 merge `6569e36e019adf4b890d04f163091f04eec6020b`; focused + full CI GREEN |
...
| R-HTTP-01 | bounded body/depth/timeouts, no unsafe serialization under locks | ChatGPT | CI | SOFTWARE CONTRACT COMPLETE / physical endurance separate | PR #107 merge `6ae86b09294b3e3a2c8a8eaf085ce5c35c69cf74` locks component-wide nonblocking spinlock use; PR #110 merge `9e14f9e1d43c961a6f3d5befd49389c7df94c829` locks bounded raw-body/parser ownership; existing config-import safety contract retained |
...
| R-OPS-01 | operator vs engineering product model | ChatGPT | CI/UI QA | SOFTWARE VERIFIED / physical UI qualification remains | `operator_product_suite_source_contract.py`, `operator_endpoint_scope_source_contract.py`, `product_engineering_access_source_contract.py`; Engineering-only request gating and sanitized operator APIs |
| R-OPS-02 | truthful source/evidence/age and no fabricated zero | ChatGPT | CI/UI QA | SOFTWARE VERIFIED | `telemetry_source_contract.py` + power-flow/operator contracts + config/status null handling; physical UI qualification remains separate |
| R-WAVE-01 | no recurring LCD sweep/reload/flicker | Claude/site | physical QA | SHORT PASS | #87/#27; only exact accepted candidate identity |
| R-WAVE-02 | Alarms opens / touch remains responsive | Claude/site | physical QA | SHORT PASS | #87/#27; only exact accepted candidate identity |
| R-WAVE-03 | >=20 kB realistic DMA headroom | Claude/site | physical QA | PASS ON CURRENT CANDIDATE | #87/#27 |
| R-WAVE-04 | uninterrupted >=4 h exact-image soak | Claude/site | physical QA | INCOMPLETE: ~2 h/121 samples clean then USB hub lost | #87/#27; shorter runs are not additive |
| R-WAVE-05 | backend parity/recovery on accepted source | Claude/site + ChatGPT | physical + API QA | PENDING AFTER FINAL SOAK | #25/#87 |
| R-WAVE-06 | persistence/ARM save-readback-reboot/failure matrix | Claude/site + ChatGPT | physical QA | PENDING AFTER FINAL SOAK | #26/#87 |
| R-OTA-01 | secure rollback-safe web OTA software | ChatGPT | GitHub Actions | SOFTWARE COMPLETE ON HELD BASELINE | #50 / PR #52; do not move frozen `phase1-fix` just to merge stale head |
...
| R-FAT-04 | communication-loss/fail-safe/endurance FAT | ChatGPT plan + Claude | physical/Owner | PENDING | #83; all three qualified Modbus connection modes must be exercised |
...
| R-GOV-02 | exact-head/zero-behind/expected-head merge gates | ChatGPT | GitHub Actions | ACTIVE GOVERNANCE | GATES.yaml / #93; PRs #107/#108/#110 merged only after live target compare + expected-head guards |
| R-GOV-03 | evidence traceability / no cross-tree physical PASS | ChatGPT | release QA | ACTIVE GOVERNANCE | #91; physical candidate/source/artifact identities are preserved independently of `dev` |
...
5. This matrix is the requirements snapshot; live exact evidence is synchronized through #91 and promotion/stale-PR hygiene through #93.
