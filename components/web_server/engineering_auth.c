...
static void hash_password(const char *password, const uint8_t salt[16], uint8_t out[32])
{
    ...
    mbedtls_sha256_finish(&ctx, current);
    for (unsigned round = 1; round < AUTH_HASH_ROUNDS; ++round) {
        mbedtls_sha256_starts(&ctx, 0);
        mbedtls_sha256_update(&ctx, current, sizeof(current));
        mbedtls_sha256_update(&ctx, salt, 16);
        mbedtls_sha256_finish(&ctx, current);
    }
...
static esp_err_t persist_credentials(const char *password)
{
    ...
    hash_password(password, s_salt, s_hash);
    nvs_handle_t handle;
...
static bool verify_password(const char *password)
{
    ...
    hash_password(password, s_salt, candidate);
    return constant_time_equal(candidate, s_hash, sizeof(candidate));
}
...
static void new_session(void)
{
    ...
    for (size_t i = 0; i < sizeof(raw); ++i) snprintf(&s_session_token[i * 2], 3, "%02x", raw[i]);
...
{
    if (!request || !out || request->content_len <= 0 || request->content_len > AUTH_BODY_MAX) return ESP_ERR_INVALID_SIZE;
...
static esp_err_t login_post(httpd_req_t *request)
{
    ...
    cJSON *json = NULL;
    if (receive_json(request, &json) != ESP_OK) return httpd_resp_send_err(request, HTTPD_400_BAD_REQUEST, "Invalid login body");
    cJSON *password = cJSON_GetObjectItemCaseSensitive(json, "password");
    bool valid = cJSON_IsString(password) && password->valuestring && verify_password(password->valuestring);
    cJSON_Delete(json);
    if (!valid) {
        s_failed_attempts++;
...
{
    if (!engineering_auth_is_authorized(request)) return engineering_auth_require(request);
    cJSON *json = NULL;
    if (receive_json(request, &json) != ESP_OK) return httpd_resp_send_err(request, HTTPD_400_BAD_REQUEST, "Invalid password body");
    ...
    bool valid = cJSON_IsString(current) && cJSON_IsString(next) && current->valuestring && next->valuestring &&
                 verify_password(current->valuestring) && strlen(next->valuestring) >= 10 && strlen(next->valuestring) <= 64;
    esp_err_t save_err = valid ? persist_credentials(next->valuestring) : ESP_ERR_INVALID_ARG;
    cJSON_Delete(json);
...
esp_err_t engineering_auth_init(void)
{
    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK) err = nvs_get_blob(handle, AUTH_HASH_KEY, s_hash, &hash_size);
        nvs_close(handle);
        if (err == ESP_OK && salt_size == sizeof(s_salt) && hash_size == sizeof(s_hash)) {
