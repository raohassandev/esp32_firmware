...
static esp_err_t css_handler(httpd_req_t *request)
{
    static const asset_getter_t assets[] = {
        ...
        web_assets_ota_css,
        /* Authoritative product presentation layer. Keep this last. */
...
static esp_err_t js_handler(httpd_req_t *request)
{
    static const asset_getter_t assets[] = {
        ...
        web_assets_ota_js,
        /* DOM-only industrial hierarchy/status ownership. Keep this last. */
...
esp_err_t web_server_start(void)
{
    ...
    /* Current web registration sources declare 46 routes including secure OTA
     * and the two fail-closed inverter assignment-manifest handlers. Keep four
     * spare slots so the next reviewed endpoint cannot silently take the server
     * to a zero-headroom start failure. The derived source contract recomputes
     * this count on every relevant change. */
