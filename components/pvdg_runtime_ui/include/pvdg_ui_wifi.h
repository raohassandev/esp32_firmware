...
typedef struct {
    ...
    uint8_t channel;
    uint8_t auth_mode;
    char security[PVDG_UI_TEXT_SMALL];
    bool connected;
    bool configured_primary;
    bool configured_fallback;
    bool secure;
    bool supported;
} pvdg_ui_wifi_network_t;
...
typedef struct {
    ...
    uint32_t generation;
    uint8_t count;
    pvdg_ui_wifi_network_t networks[PVDG_UI_WIFI_MAX_NETWORKS];
} pvdg_ui_wifi_scan_t;

typedef struct {
    void (*request_scan)(void *user);
    void (*request_reconnect)(void *user);
    void (*submit_config)(const pvdg_ui_wifi_config_t *config,
                          bool primary_changed,
                          void *user);
    void *user;
} pvdg_ui_wifi_callbacks_t;
