...
# 1. Only settled carrying modes may drive the plant. Anything else is not stable.
...
# 2. The policy rejects unsettled input before any command is calculated, so a
#    changeover produces zero rather than a ramped-down remnant.
...
# 3. Measured-source changes remain debounced and clear the previous stable state.
...
# 4. Strong source evidence has a second admission gate. Every distinct carrying
#    mode (grid, generator, island, synchronized) must earn a fresh uninterrupted
#    stabilization dwell. This prevents GRID READY from being inherited by a
#    generator immediately after transfer, and vice versa.
...
# 5. Executable coverage proves both immediate zeroing and fresh dwell across
#    actual source-mode changes; slow ramping may not soften either safety rule.
