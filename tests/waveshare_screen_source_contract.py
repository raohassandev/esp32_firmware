...
def main() -> None:
    ...
    # The provider consumes the shared source-detection STATUS snapshot and asks
    # the Core-owned fail-closed attribution helper for the operator verdict.
    # Board code must never assume or recreate a struct field for that verdict.
    ...
    assert "source.attributed_to" not in product_provider
