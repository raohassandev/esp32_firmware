...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="secure-padded unsigned app from the signed-OTA build profile",
    )
    parser.add_argument(
        "--key", required=True, type=Path,
        help="RSA-3072 private signing key outside the repository",
    )
    parser.add_argument("--output", required=True, type=Path, help="destination signed .bin")
    parser.add_argument(
        "--manifest", type=Path,
        help="release manifest path; defaults beside output",
    )
    ...
    if key_is_inside_repo(repo, key):
        fail(
            ...
            "Move it to controlled external storage."
        )
    if source == output:
    ...
    if source.stat().st_size >= OTA_SLOT_BYTES:
        fail(
            f"Unsigned application is already too large for the "
            f"0x{OTA_SLOT_BYTES:X}-byte OTA slot."
        )
    ...
    # ESP-IDF owns the exact RSA-PSS signature block format. The key stays in
    # controlled external storage and is never copied into the repository or
    # release manifest.
    subprocess.run(
        [
            args.idf_py,
    ...
    if output.stat().st_size <= source.stat().st_size:
        fail(
            "Signed image did not grow; expected an appended Secure Boot v2 "
            "signature block."
        )
    ...
    # Verify the produced binary before release packaging. ESP-IDF accepts the
    # matching public/private key for this verification command.
    subprocess.run(
        [
            args.idf_py,
