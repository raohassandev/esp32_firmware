...
- [ ] **NETWORK / SITE WAIT — repeat local Waveshare meter diagnostic.** The 2026-09-15 root `dev` image built and booted on the connected Waveshare ESP32-S3-Touch-LCD-5 but lacked its LCD driver; the exact Waveshare product candidate was subsequently restored. Commissioned STA SSID `Rao` was absent, so no ZLAN/EM-500 Modbus response was observed. Restore an approved reachable network and verify the real gateway endpoint, unit ID and read-only meter values. See `docs/LOCAL_PHYSICAL_VALIDATION_2026-09-15.md` and `docs/WAVESHARE_BLACK_SCREEN_RESTORE_2026-09-15.md`. This diagnostic does not satisfy any frozen release-candidate gate below.
